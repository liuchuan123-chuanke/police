...
public class CustomRedisTokenStore implements TokenStore {
                ...
                if (validitySeconds > 0) {
                    ...
                    if (expiresRatio <= securityProperties.getAuth().getRenew().getTimeRatio()) {
                        ...
                        storeAccessToken(oAuth2AccessToken, auth2Authentication,
                                securityProperties.getAuth().getRenew().getEnable());
                    }
                }
            }
        }
    ...
    private boolean checkRenewClientId(String clientId) {
        ...
        if (!CollectionUtils.isEmpty(includeClientIds)) {
            result = includeClientIds.contains(clientId);
        } else if (!CollectionUtils.isEmpty(exclusiveClientIds)) {
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        storeAccessToken(token, authentication, securityProperties.getAuth().getRenew().getEnable());
    }

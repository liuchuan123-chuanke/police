...
import com.imeng.web.model.MyClientDetails;
 ...
 */
@Slf4j
public class RedisClientDetailsService extends JdbcClientDetailsService implements ZltClientDetailsService {
    ...
    public void loadAllClientToCache() {
        ...
        list.parallelStream().forEach(client -> redisTemplate.opsForValue().set(clientRedisKeyWithScope(client.getClientId(),client.getScope().iterator().next()), client));
    }
    ...
    @Override
    public ClientDetails loadClientByKeyGen(String clientId, String scope) {
        // 先从redis获取
        ClientDetails clientDetails = (ClientDetails) redisTemplate.opsForValue().get(clientRedisKeyWithScope(clientId,scope));
        if (clientDetails == null) {
            clientDetails = cacheAndGetClientWithScope(clientId,scope);
        }
        return clientDetails;
    }
     ...
     * @return
     */
    private ClientDetails cacheAndGetClientWithScope(String clientId,String scope) {
        // 从数据库读取
        Client clientDetails = null;
        try {
            clientDetails = clientService.queryClient(clientId,scope);
            if (clientDetails != null) {
                // 写入redis缓存
                redisTemplate.opsForValue().set(clientRedisKeyWithScope(clientId,scope), new MyClientDetails(clientDetails));
                log.info("缓存clientId:{},{}", clientId, clientDetails);
            }
        } catch (NoSuchClientException e) {
            log.error("clientId:{},{}", clientId, clientId);
        ...
        return new MyClientDetails(clientDetails);
    }
}
